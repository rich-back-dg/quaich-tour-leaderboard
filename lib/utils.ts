...
export const sortGroupedDivisionsByRanking = (a: string[], b: string[]): number => {
  // Find the ranks of the first division in each group
  const rankA = divisionRankings.find((division) => division.name === a[0])?.rank || 0;
  const rankB = divisionRankings.find((division) => division.name === b[0])?.rank || 0;
